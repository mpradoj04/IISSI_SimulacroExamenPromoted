...
import { create, getRestaurantCategories, getAnyPromoted } from '../../api/RestaurantEndpoints'
...
export default function CreateRestaurantScreen ({ navigation, route }) {
  ...
  const [anyPromoted, setAnyPromoted] = useState()

  const initialRestaurantValues = { name: null, description: null, address: null, postalCode: null, url: null, shippingCosts: null, email: null, phone: null, restaurantCategoryId: null, promoted: null }
  const validationSchema = yup.object().shape({
    ...
    promoted: yup
      .boolean()
      .required()
      .oneOf([false, anyPromoted === null], 'There is already one of your restaurants promoted')
  })
  ...
  useEffect(() => {
    async function fetchAnyPromoted () {
      try {
        const restaurantPromoted = await getAnyPromoted()
        setAnyPromoted(restaurantPromoted)
      } catch (error) {
        console.log(error)
      }
    }
  ...
  const createRestaurant = async (values) => {
    ...
    try {
      ...
      showMessage({
        message: `There was an error while creating restaurant. ${error}`,
        type: 'error',
        style: GlobalStyles.flashStyle,
        titleStyle: GlobalStyles.flashTextStyle
      })
